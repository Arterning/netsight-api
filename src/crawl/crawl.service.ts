...
import puppeteer, { HTTPResponse as PuppeteerHTTPResponse } from 'puppeteer';
import { chromium, Page as PlaywrightPage, Response as PlaywrightResponse } from 'playwright';
import { URL } from 'url';
...
export class CrawlService {
  ...
  private async crawlPageWithPuppeteer(url: string, proxy?: string, headless?: boolean): Promise<CrawlPageResult> {
    ...
    let response: PuppeteerHTTPResponse | null = null;
    try {
  ...
  private async crawlPageWithPlaywright(url: string, proxy?: string, headless?: boolean): Promise<CrawlPageResult> {
    ...
    try {
      ...
      return {
        ...
        vulnerabilities: JSON.stringify([{
          type: 'Crawl Error',
          description: `Playwright failed to navigate to the page. Error: ${error instanceof Error ? error.message : String(error)}`,
          severity: 'Critical'
        }]),
        screenshotBase64: null,
        apiRequests: [],
